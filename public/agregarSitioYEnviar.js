...
export async function agregarSitioYEnviar(dataSitio) {
    ...
    const url = `http://localhost:1234/sitios/${dataSitio.idWiki}`;
    ...
    if (existe.status === 404) {
        ...
        try {
            const response = await fetch('http://localhost:1234/sitios', options);
    ...
    try {
        const response = await fetch(`http://localhost:1234/sitios/${dataSitio.idWiki}`, options);
        const json = await response.json();
